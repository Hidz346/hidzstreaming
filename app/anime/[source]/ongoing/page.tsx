...
export default function AnimeOngoingPage() {
  ...
  const fetchOngoing = async (p: number) => {
    setLoading(true);
    try {
      ...
      const mapped = items.map((item: any) => ({
        ...
        status: 'ONGOING',
        year: '2026',
        views: item.episodes || item.episode || '13 Eps'
      }));
      setList(mapped);
    } catch (error) {
      console.error("Failed to fetch ongoing", error);
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="flex-1 flex flex-col min-h-screen text-white pb-24 font-sans w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-4">
      ...
      <div className="flex items-center gap-3 mb-8">
        ...
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg sm:text-xl font-bold text-white">Anime Ongoing</h1>
        </div>
      </div>
      ...
      <div className="w-full">
        {loading ? (
          ...
          <div className="text-center py-20 text-zinc-500">Belum ada anime ongoing.</div>
        ) : (
          <>
