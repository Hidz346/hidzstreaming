...
export default function HomeIdPage() {
  ...
  useEffect(() => {
    ...
    try {
      const historyStr = localStorage.getItem('hidz_history');
      if (historyStr) {
    ...
    const fetchData = async () => {
      try {
        const [dhRes, wtRes, comicRes, novelRes, comicNewRes, comicColorRes] = await Promise.all([
        ...
        if (novelRes) {
          const vResults = novelRes.data?.results || novelRes.data || novelRes.result?.items || [];
          setNovels(vResults.map((n: any) => ({
            ...n,
      ...
      <div className="flex-1 min-w-0">
        ...
        {loading ? (
          ...
          <div className="flex flex-col gap-10">
            ...
            {novels.length > 0 && (
              <section>
                <WidgetTitle title="HidzNovel Terbaru" href="/novel" />
                <AnimeList items={novels.slice(0, 10)} />
              </section>
            )}
